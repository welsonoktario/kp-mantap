...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    edit: function edit(item, index, button) {
      this.selectedAktivitas = this.items.find(function (i) {
        return i.id === item.id;
      });
    ...
    hapus: function hapus(item, index, button) {
      ...
      var selected = this.items.findIndex(function (a) {
        return a.id === item.id;
      });
      this.$bvModal.msgBoxConfirm("Apakah anda yakin menghapus transaksi ini?", {
        ...
        if (value) {
          window.axios["delete"]("/aktivitas/".concat(item.id)).then(function (res) {
            if (res.status === 200) {
              _this.items.splice(selected, 1);

              _this.toast('Aktivitas', 'Berhasil menghapus aktivitas');
            } else {
              _this.toast('Aktivitas', 'Gagal menghapus aktivitas', 'danger');
            }
          });
        }
      })["catch"](function (err) {
      ...
      window.axios.get('/user').then(function (res) {
        ...
        window.axios.get('/aktivitas', {
          ...
          data.data.forEach(function (a) {
            var total = Number(a.total_pemasukan) - Number(a.total_pengeluaran);

            _this.aktivitas.push({
              id: a.id,
