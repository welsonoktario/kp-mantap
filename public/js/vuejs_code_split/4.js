...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    edit: function edit(item, index, button) {
      this.selectedKategori = this.items.find(function (i) {
        return i.id === item.id;
      });
    ...
    del: function del(item, index, button) {
      ...
      this.$bvModal.msgBoxConfirm("Apakah anda yakin menghapus kategori ".concat(item.nama, "?"), {
        title: 'Peringatan',
        size: 'sm',
        buttonSize: 'sm',
        okVariant: 'danger',
        ...
        if (value) {
          window.axios["delete"]("/kategori/".concat(item.id)).then(function (res) {
            if (res.status === 200) {
              _this.items.splice(selected, 1);

              _this.toast('Kategori', 'Berhasil menghapus kategori');
            } else {
              _this.toast('Kategori', 'Gagal menghapus kategori', 'danger');
            }
          });
        }
      })["catch"](function (err) {
    ...
    detail: function detail(item, index, e) {
      this.selectedKategori = this.items.find(function (i) {
        return i.id === item.id;
      });
      this.$router.push({
                ...
                fn: function(row) {
                  return [
                    ...
                    _vm.$parent.$data.user.role === "Bendahara"
                      ...
                      : _vm._e(),
                    _vm._v(" "),
                    _vm.items && _vm.items[row.index].transaksi_count === 0
                      ? _c(
                          "b-button",
                          {
                            staticClass: "mr-1",
