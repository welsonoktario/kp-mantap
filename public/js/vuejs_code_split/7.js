...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  mounted: function mounted() {
    ...
    window.axios.get("/transaksi-all?id=".concat(this.$route.params.id)).then(function (res) {
    ...
    tambah: function tambah() {
      ...
      window.axios.post('/transaksi-pilih', {
        ...
        if (res.status === 200) {
          ...
          _this2.toast('Aktivitas', 'Berhasil menambah transaksi ke aktivitas');
        } else {
          _this2.toast('Aktivitas', 'Gagal memuat data transaksi', 'danger');
        }
      });
    }
  }
  ...
  methods: {
    loadTransaksi: function loadTransaksi() {
      ...
      window.axios.get('/user').then(function (res) {
        _this.user = res.data;
        window.axios.get("/aktivitas/".concat(id), {
          params: {
            page: current_page,
          ...
          _this.transaksi.data = res.data.data.data;
          _this.aktivitas = res.data.kegiatan;
          _this.transaksi.meta = {
        ...
        [
          ...
          _vm.transaksi.data
            ? _c("DataTable", {
                attrs: {
                  fields: _vm.columns,
                  items: _vm.transaksi.data,
                  meta: _vm.transaksi.meta,
                  "is-detail": "Kegiatan"
                },
                on: {
